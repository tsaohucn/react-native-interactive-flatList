...
export default class ComicBook extends Component {
  ...
  UNSAFE_componentWillMount() {
    this.gestureHandlers = PanResponder.create({
      // Setter
      onStartShouldSetPanResponderCapture: evt => false, // 開始觸碰，是否捕捉成為響應者，手勢放在flatlist裏根本不會執行
      ...
      onPanResponderTerminationRequest: evt => false, // 有外層其他響應者，是否釋放響應權，最外層不用設
      // Handler
  ...
  onPanResponderGrant = (evt, gestureState) => {
    if (evt.nativeEvent.touches) {
      this.clickX = evt.nativeEvent.touches[0].pageX
      this.clickY = evt.nativeEvent.touches[0].pageY
    }
  }
